        ...
        <table class="mdui-table mdui-table-selectable">
            <thead>
                <tr>
                    ...
                    <th class="mdui-table-col-numeric">链接</th>
                    <th class="mdui-table-col-numeric">图片URL</th>
...
<script>
    !function () {
        ...
        fetch("../?type=func&target=get_nav", {
            method: "POST"
        }).then(v => v.json()).then(v => {
            var data = Object.values(v.data);
            data.forEach(v => {
                mdui.$("tbody").append(`<tr><td>${v.id}</td><td>${v.name}</td><td>${v.url}</td></tr>`);
                mdui.mutation();
                mdui.updateTables();
            });
            function arrange() {
            ...
            mdui.$("div>button:nth-child(2)").on("click", function () {
                window.top.writeHtml(
                    `<div class="mdui-dialog"><div class="mdui-dialog-title">添加</div><div class="mdui-dialog-content"><div class="mdui-textfield mdui-textfield-floating-label"><label class="mdui-textfield-label">名称</label><input class="mdui-textfield-input" /></div><div class="mdui-textfield mdui-textfield-floating-label"><label class="mdui-textfield-label">链接</label><input class="mdui-textfield-input" /></div></div><div class="mdui-dialog-actions"><a href="javascript:void(0)" class="mdui-btn mdui-ripple mdui-text-color-primary">关闭</a><a href="javascript:void(0)" class="mdui-btn mdui-ripple mdui-text-color-primary">添加</a></div></div>`,
                    function (p) {
                        var is_click = false;
            ...
            mdui.$("div>button:nth-child(3)").on("click", function () {
                window.top.writeHtml(
                    `<div class="mdui-dialog"><div class="mdui-dialog-title">编辑</div><div class="mdui-dialog-content"><div class="mdui-textfield mdui-textfield-floating-label"><label class="mdui-textfield-label">名称</label><input class="mdui-textfield-input" /></div><div class="mdui-textfield mdui-textfield-floating-label"><label class="mdui-textfield-label">链接</label><input class="mdui-textfield-input" /></div></div><div class="mdui-dialog-actions"><a href="javascript:void(0)" class="mdui-btn mdui-ripple mdui-text-color-primary">关闭</a><a href="javascript:void(0)" class="mdui-btn mdui-ripple mdui-text-color-primary">保存</a></div></div>`,
                    function (p) {
                        var id = mdui.$(".mdui-table-row-selected>td:nth-child(2)").text()
                        var name = mdui.$(".mdui-table-row-selected>td:nth-child(3)").text()
                        var url = mdui.$(".mdui-table-row-selected>td:nth-child(4)").text()
                        var is_click = false;
                        var x = new p.w.mdui.Dialog(p.jq[0]);
                        var input = p.jq.find("input")
                        x.open();
                        ...
                        var s = setInterval(() => {
                            if (x.state == "closed") {
                                ...
                                if (is_click) {
                                    if (input[0].value) {
                                        mdui.$.ajax({
                                            url: "../?type=func&target=change_nav",
                                            method: "POST",
                                            data: {
                                                id,
                                                name: input[0].value,
            ...
            mdui.$("div>button:nth-child(4)").on("click", function () {
                window.top.mdui.confirm("删除后不可恢复，确认继续？", "删除", function () {
                    ...
                    all.each((id, e) => {
                        mdui.$.ajax({
                            url: "../?type=func&target=delete_nav",
                            method: "POST",
                            data: { id: mdui.$(e).text() },
                            success: function () {
                                deleteCount--;
